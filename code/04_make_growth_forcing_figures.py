...
chain_core=df.dropna(subset=['Z1_Hc60','D_C_frac_2008_2015','Delta_Fu_post_minus_pre']).copy()
...
for lab,marker in zip(labels,markers):
    ...
    if len(dd)==0:
        continue
    sc=axA.scatter(dd['Z1_Hc60'], dd['D_C_frac_2008_2015'],
...
for _,r in chain_core.iterrows():
    axA.annotate(r['code'],(r['Z1_Hc60'],r['D_C_frac_2008_2015']),textcoords='offset points',xytext=(5,5),fontsize=8)
if len(chain_core)>=3:
    coef=np.polyfit(chain_core['Z1_Hc60'], chain_core['D_C_frac_2008_2015'],1)
    xx=np.linspace(chain_core['Z1_Hc60'].min(),chain_core['Z1_Hc60'].max(),100)
    axA.plot(xx,coef[0]*xx+coef[1],lw=1)
    rA=chain_core['Z1_Hc60'].corr(chain_core['D_C_frac_2008_2015'])
    axA.set_title(f"A. $Z_1$ vs $D_C$ (r={rA:.2f}, n={len(chain_core)})")
else:
...
if len(chain_ext)>=3:
    ...
    rB=chain_ext['D_C_frac_2008_2015'].corr(chain_ext['Delta_Fu_post_minus_pre'])
    axB.set_title(f"B. $D_C$ vs $\\Delta F_u$ (r={rB:.2f}, n={len(chain_ext)})")
else:
...
fig.savefig(FIG/"fig6_Z1_DC_DeltaFu_chain.png",dpi=240); plt.close(fig)
...
plot_xy('D_C_frac_2008_2015','Delta_Fu_post_minus_pre',
        r"$D_C$: consumption depth-duration below trend",
        r"$\Delta F_u=\bar g_{2011-2013}-\bar g_{2004-2007}$ (pp)",
        "Deterioration in growth slope vs crisis depth-duration",
        "fig6_DC_DeltaFu_coupling.png")
